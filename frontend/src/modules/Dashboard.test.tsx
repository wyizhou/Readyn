...
describe('Dashboard (P2 redesign)', () => {
  it('shows the all-sport core with source-transparency badges', () => {
    ...
    expect(within(loadMetrics).getByText('负荷决策')).toBeInTheDocument()
    expect(within(loadMetrics).getByText('先恢复，再推进强度。当前 TSB 为负，且健康数据窗口待补全。')).toBeInTheDocument()
    expect(within(loadMetrics).getByText(/当前健康窗口不足/)).toBeInTheDocument()
    expect(within(loadMetrics).getByText('健康数据待补全')).toBeInTheDocument()
    expect(within(loadMetrics).queryByText('今日状态')).not.toBeInTheDocument()
    expect(within(loadMetrics).queryByText('状态均衡')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '安排轻松训练' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '复制公式' })).not.toBeInTheDocument()
    ...
    const trendDetail = screen.getByRole('region', { name: '体能趋势明细' })
    expect(within(trendDetail).getByLabelText('如何计算')).toBeInTheDocument()
    expect(within(trendDetail).getByText('Trainalyze 自算')).toBeInTheDocument()
    expect(within(trendDetail).getByText('近 6 周 · 体能 / 疲劳 / 状态')).toBeInTheDocument()
    expect(within(trendDetail).getByRole('tablist', { name: '体能趋势视图' })).toBeInTheDocument()
    expect(within(trendDetail).getByRole('tab', { name: '每日', selected: true })).toBeInTheDocument()
    expect(within(trendDetail).getByRole('tab', { name: '来源', selected: false })).toBeInTheDocument()
    expect(within(trendDetail).getByRole('tab', { name: '公式', selected: false })).toBeInTheDocument()
    expect(within(trendDetail).getByRole('tabpanel', { name: '每日' })).toBeInTheDocument()
    expect(within(trendDetail).getByLabelText('趋势图例')).toBeInTheDocument()
    expect(within(trendDetail).queryByRole('region', { name: '来源证据' })).not.toBeInTheDocument()
    expect(within(trendDetail).queryByRole('region', { name: '公式摘要' })).not.toBeInTheDocument()
  ...
  it('switches the trend tabs between daily, source, and formula views', async () => {
    const user = userEvent.setup()
    renderDash()
    const trendDetail = screen.getByRole('region', { name: '体能趋势明细' })

    await user.click(within(trendDetail).getByRole('tab', { name: '来源' }))
    expect(within(trendDetail).getByRole('tab', { name: '来源', selected: true })).toBeInTheDocument()
    const sourceEvidence = within(trendDetail).getByRole('region', { name: '来源证据' })
    ...
    expect(within(trendDetail).queryByRole('region', { name: '公式摘要' })).not.toBeInTheDocument()
    expect(document.getElementById('trend-panel-daily')).toHaveAttribute('hidden')

    await user.click(within(trendDetail).getByRole('tab', { name: '公式' }))
    expect(within(trendDetail).getByRole('tab', { name: '公式', selected: true })).toBeInTheDocument()
    const formulaSummary = within(trendDetail).getByRole('region', { name: '公式摘要' })
    ...
    await user.click(within(trendDetail).getByRole('tab', { name: '每日' }))
    expect(within(trendDetail).getByRole('tab', { name: '每日', selected: true })).toBeInTheDocument()
    expect(within(trendDetail).getByLabelText('趋势图例')).toBeInTheDocument()
    expect(within(trendDetail).queryByRole('region', { name: '来源证据' })).not.toBeInTheDocument()
    expect(within(trendDetail).queryByRole('region', { name: '公式摘要' })).not.toBeInTheDocument()
