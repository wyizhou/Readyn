...
首页 / `index.html` 已确认对应当前 `dashboard` 总览入口。总览页最小骨架已覆盖负荷指标、体能趋势、来源证据、公式说明和下一次训练建议空态；当前仍不是完整 Open Design 视觉重写。总览页核心结构已补充可测试的命名区域标识，包括页面头部、负荷指标、体能趋势、来源证据/公式说明和下一次训练建议。页面头部已补充“今日负荷模型”、决策型标题、说明文案和 Garmin/健康窗口状态标签；无连接时明确显示未连接/待补全，不伪造同步状态。负荷指标卡已对齐到 ATL、CTL、TSB、A:C 和 Easy TRIMP；其中 Easy TRIMP 当前没有真实数据字段，界面显示 `—` / 待算法字段，不伪造数值。体能趋势卡已补充“每日 / 来源 / 公式”最小 tabs 结构，来源证据和公式摘要已挂到对应 tab，并明确 Garmin 中国数据来源、Trainalyze 自算负荷模型和未接入来源边界。下一次训练建议卡已补充“强度 / 原因 / 后续”最小证据结构，仅使用现有 `workout` 字段；无 workout 数据时继续显示空态。
