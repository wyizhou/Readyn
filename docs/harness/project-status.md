...
首页 / `index.html` 已确认对应当前 `dashboard` 总览入口。总览页最小骨架已覆盖负荷指标、体能趋势、来源证据、公式说明和下一次训练建议空态；当前仍不是完整 Open Design 视觉重写。总览页核心结构已补充可测试的命名区域标识，包括负荷指标、体能趋势、来源证据/公式说明和下一次训练建议。
