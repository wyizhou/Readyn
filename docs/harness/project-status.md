...
首页 / `index.html` 已确认对应当前 `dashboard` 总览入口。总览页最小骨架已覆盖负荷指标、体能趋势、来源证据、公式说明和下一次训练建议空态；当前仍不是完整 Open Design 视觉重写。总览页核心结构已补充可测试的命名区域标识，包括负荷指标、体能趋势、来源证据/公式说明和下一次训练建议。负荷指标卡已对齐到 ATL、CTL、TSB、A:C 和 Easy TRIMP；其中 Easy TRIMP 当前没有真实数据字段，界面显示 `—` / 待算法字段，不伪造数值。体能趋势卡已补充最小来源证据和公式摘要结构，明确 Garmin 中国数据来源、Trainalyze 自算负荷模型和未接入来源边界。
