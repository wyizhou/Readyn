...
- [~] 将当前 Dashboard 调整为 Open Design 的负荷总览结构，包括指标卡、趋势图、来源证据、公式说明、下一次训练建议。（已完成结构标识切片和负荷指标卡字段切片；完整趋势/来源/公式交互和视觉对齐仍保留。）
...
暂定：继续拆分 Dashboard 负荷总览内容对齐，下一步只选择一个具体可验证差异，例如趋势来源/公式呈现或下一次训练建议结构。
...
- P0 主流程骨架与空态审计已完成。
- 总览页结构标识和负荷指标卡字段切片已完成，但 Dashboard 仍是 `partial`。
