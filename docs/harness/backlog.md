...
- [~] 将当前 Dashboard 调整为 Open Design 的负荷总览结构，包括指标卡、趋势图、来源证据、公式说明、下一次训练建议。（已完成页面头部骨架、结构标识、负荷指标卡字段、体能趋势 tabs、趋势来源证据和公式摘要、下一次训练建议结构切片；完整视觉对齐仍保留。）
...
- P0 主流程骨架与空态审计已完成。
- 总览页页面头部、结构标识、负荷指标卡字段、体能趋势 tabs、趋势来源证据、公式摘要和下一次训练建议结构切片已完成，但 Dashboard 仍是 `partial`。
