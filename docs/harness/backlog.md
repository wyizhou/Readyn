...
- [~] 将当前 Dashboard 调整为 Open Design 的负荷总览结构，包括指标卡、趋势图、来源证据、公式说明、下一次训练建议。（已完成结构标识、负荷指标卡字段、趋势来源证据和公式摘要、下一次训练建议结构切片；完整 tabs 交互和视觉对齐仍保留。）
...
暂定：继续拆分 Dashboard 负荷总览内容对齐，下一步只选择一个具体可验证差异，例如趋势 tabs 交互或视觉密度对齐。
...
- P0 主流程骨架与空态审计已完成。
- 总览页结构标识、负荷指标卡字段、趋势来源证据、公式摘要和下一次训练建议结构切片已完成，但 Dashboard 仍是 `partial`。
